...
namespace Microsoft.NetworkMonitor.Automation
{
    ...
    public class CommandLineArguments
    {
        ...
        /// <summary>
        /// Name and path of the file where the output must be generated
        /// </summary>
        ...
        public bool ParseCommandLineArguments(string[] args)
        {
            ...
            else
            {
                ...
                while (arguments.Count > 0)
                {
                    ...
                    if (cmd.StartsWith("/d", StringComparison.CurrentCultureIgnoreCase))
                    {
                        if (arguments.Count > 1)
                        {
                            ...
                            if (!arg.StartsWith("/c", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/f", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/o", StringComparison.CurrentCultureIgnoreCase))
                            {
                    ...
                    else if (cmd.StartsWith("/c", StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        {
                            string arg = arguments[1].Trim();
                            if (!arg.StartsWith("/d", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/f", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/o", StringComparison.CurrentCultureIgnoreCase))
                            {
                    ...
                    else if (cmd.StartsWith("/f", StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        {
                            string arg = arguments[1].Trim();
                            if (!arg.StartsWith("/c", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/d", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/o", StringComparison.CurrentCultureIgnoreCase))
                            {
                    ...
                    else if(cmd.StartsWith("/o", StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        {
                            string arg = arguments[1].Trim();
                            if (!arg.StartsWith("/c", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/d", StringComparison.CurrentCultureIgnoreCase) && !arg.StartsWith("/f", StringComparison.CurrentCultureIgnoreCase))
                            {
                                this.outputFile = arg;
                                skip = 2;
                            }
                ...
                if ((!this.displayFilter.ToLower().Contains("http and tcp")) && (!this.displayFilter.ToLower().Contains("tcp and http")))
                {
                    if (this.displayFilter.Equals(string.Empty))
                    {
                        //Use this default display filter if one is not passed

                        //Was earlier using this filter however this is slightly inaccurate so using this new one as the default filter
                        //this.displayFilter = "tcp and http";
                        //this.displayFilter = "(tcp.port==80  and not(contains(Description, 'PayloadLen=0')) and not SSDP and not TLS and not SSL)";
                        this.displayFilter = "(tcp  and not(contains(Description, 'PayloadLen=0')) and not SSDP and not TLS and not SSL)";

                    }
                    else
                    {
                        //We will have access to the actual display filter here. In case it needs to be modified, it can be done here
                        //There is a possiblity that this parser will fail if an incorrect filter is passed. Although I have tested this scenario, but will have to check that possibility exhaustively
                        //this.displayFilter = "and tcp and http";
                    }
                }
                if (this.outputFile == null)
                {
                    this.outputFile = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + "\\" + this.CaptureFileName.Replace(System.IO.Path.GetDirectoryName(this.CaptureFileName) + "\\", "") + "_Report.htm";
                }
            }
